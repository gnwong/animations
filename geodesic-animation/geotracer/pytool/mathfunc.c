...
int invert_44(double gcov[4][4], double gcon[4][4]) {
    ...
    int invert_matrix(double Am[4][4], double Aminv[4][4]);

    int sing = invert_matrix(gcov, gcon);
    if (sing) {
      fprintf(stderr, "! singular 4x4 matrix:\n");
      for (int mu = 0; mu < 4; mu++) {
        for (int nu = 0; nu < 4; nu++) {
          fprintf(stderr, "  gcov[%i][%i] = %e\n", mu, nu, gcov[mu][nu]);
        }
      }
    }
...
int LU_decompose(double A[4][4], int permute[])
{
