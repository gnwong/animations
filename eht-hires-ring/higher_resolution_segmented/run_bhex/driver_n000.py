    ...
    for dump in dumps[:-1]:
        ...
        for si in stride2o:
            for sj in stride2o:
                ...
                outfile = '/mnt/lustre/user/gnwong/images/highres/' + model + '/' + os.path.basename(dump).replace('.h5', '') + stride_string + '_n000.h5'
                ...
                args = ipole.run_ipole(dump, outfile=outfile, thetacam=60, rhigh=rhigh, munit=munit, res=res, unpol=True, onlyargs=True, strides=strides)
                args.append('--target_nturns=0')
                print(' '.join(args))
