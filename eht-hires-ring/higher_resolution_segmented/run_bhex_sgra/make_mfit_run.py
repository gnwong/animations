...
if __name__ == "__main__":
    ...
    res = 80
    for dump in dumps:
        for munit in np.logspace(20, 30, 11):
            outfile = ofb + os.path.basename(dump).replace('.h5', '') + f'_{int(np.log10(munit))}.h5'
            args = ipole.run_ipole(dump, outfile=outfile, rhigh=rhigh, munit=munit, res=res, unpol=True, thetacam=60, onlyargs=True, sigma_cut_high=25.)
            print(' '.join(args))
        break
